...
import type { Capacitacion, CapacitacionFormData } from '@/types/capacitaciones';
...
const CapacitacionSingle: React.FC<CapacitacionSingleProps> = ({ capacitacionId, onBack }) => {
  ...
  const fetchCapacitacion = async (): Promise<void> => {
    try {
      ...
      toast({ title: "Error", description: "Error al cargar la capacitación", variant: "destructive" });
      handleBack();
    } finally {
      setLoading(false);
    }
  ...
  const fetchAsistentes = async (): Promise<void> => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Error al cargar asistentes", variant: "destructive" });
    } finally {
  ...
  const fetchTemas = async (): Promise<void> => {
    try {
      ...
      toast({ title: "Error", description: "Error al cargar temas", variant: "destructive" });
    }
  };
  ...
  const fetchEvaluaciones = async (): Promise<void> => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Error al cargar evaluaciones", variant: "destructive" });
    } finally {
  ...
  const handleSave = async (formData: CapacitacionFormData): Promise<void> => {
    try {
      ...
      toast({ title: "Error", description: "Error al actualizar la capacitación", variant: "destructive" });
    }
  };
  ...
  const handleDelete = async (): Promise<void> => {
    if (window.confirm("¿Está seguro de que desea eliminar esta capacitación?")) {
      try {
        ...
        toast({ title: "Error", description: "Error al eliminar la capacitación", variant: "destructive" });
      }
    }
  };

  const handleToggleAsistente = async (empleado: Personal): Promise<void> => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Error al actualizar asistentes", variant: "destructive" });
    }
  };

  const handleOpenEvalModal = (empleado_id: string = '', tema_id: string = '', evaluacion: Evaluacion | null = null): void => {
    if (evaluacion) {
      setEvalForm({
        empleado_id: evaluacion.empleado_id.toString(),
        tema_id: evaluacion.tema_id.toString(),
  ...
  const handleSaveEvaluacion = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    try {
      if (editEvalId) {
      ...
      toast({ title: "Error", description: "Error al guardar evaluación", variant: "destructive" });
    }
  };
  ...
  const handleDeleteEvaluacion = async (evalId: number): Promise<void> => {
    if (window.confirm('¿Eliminar esta evaluación?')) {
      try {
        ...
        toast({ title: "Error", description: "Error al eliminar evaluación", variant: "destructive" });
      }
    }
  };
  ...
  const buscarEmpleados = async (query: string): Promise<void> => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Error al buscar empleados", variant: "destructive" });
    } finally {
      setBuscando(false);
    }
  };
  ...
  const handleAgregarAsistente = async (empleado: Personal): Promise<void> => {
    try {
      ...
      toast({ title: "Error", description: "Error al agregar asistente", variant: "destructive" });
    }
  };
  ...
  const handleEliminarAsistente = async (asistente: Asistente): Promise<void> => {
    try {
      ...
      toast({ title: "Error", description: "Error al eliminar asistente", variant: "destructive" });
    }
  };
  ...
  const handleSaveInlineEval = async (): Promise<void> => {
    try {
      if (editEvalInlineId === 'new') {
        ...
        await capacitacionesService.updateEvaluacion(id, editEvalInlineId as number, inlineEvalForm);
        toast({ title: "Éxito", description: "Evaluación actualizada" });
      }
            ...
            <form onSubmit={handleSaveEvaluacion} className="space-y-4 mt-2">
                ...
                <Select value={evalForm.empleado_id} onValueChange={v => handleEvalFormChange('empleado_id', v)} required disabled={!!editEvalId}>
                  ...
                  <SelectContent>
                    {personal.filter(emp => asistentes.some(a => a.empleado_id === emp.id)).map(emp => (
                      <SelectItem key={emp.id} value={emp.id.toString()}>{emp.nombres} {emp.apellidos}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                ...
                <Select value={evalForm.tema_id} onValueChange={v => handleEvalFormChange('tema_id', v)} required disabled={!!editEvalId}>
                  ...
                  <SelectContent>
                    {temas.map(t => (
                      <SelectItem key={t.id} value={t.id.toString()}>{t.titulo}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
