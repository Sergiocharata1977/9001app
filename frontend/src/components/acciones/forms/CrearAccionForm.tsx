...
import type { AccionPrioridad } from '@/types/acciones';

interface CrearAccionFormData {
  titulo: string;
  descripcion: string;
  responsable: string;
...
const CrearAccionForm: React.FC<CrearAccionFormProps> = ({ onSubmit, onCancel, isLoading }) => {
  const [formData, setFormData] = useState<CrearAccionFormData>({
    titulo: '',
    descripcion: '',
    responsable: '',
    prioridad: 'media',
  ...
  const handleChange = (field: keyof CrearAccionFormData, value: string | number | null) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
